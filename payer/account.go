...
const accountRoute = "/x402/account"
...
func (a *Accounts) Read(ctx context.Context, address string) (AccountAnswer, error) {
	...
	if res.StatusCode != http.StatusOK {
		return AccountAnswer{}, a.statusError(res.StatusCode, body)
	}
...
func (a *Accounts) statusError(status int, body []byte) error {
	var out struct {
	...
	case http.StatusNotFound:
		...
		return fmt.Errorf(
			"the sidecar at %s has no %s route. It must be x402-celestia-rpc 0.2 or later",
			a.upstream, accountRoute)
